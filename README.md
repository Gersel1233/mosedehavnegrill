...
mailen booking men gør det pænt og ordentligt og der kommer billeder
men det er der bare ikke endnu men hvor man kan læse om det og vi
elsker det og vores personale er dygtige maden er god og vi holder
alt og skræddersyr præcis til jeres behov"* — og i samme åndedrag
*"samme ting med processen"*. **Ingen SQL.**

### Hvad prisen for beslutningen er

Siden var en forespørgselsformular fra 23/8 til 4/9. Forespørgslen
landede i tabellen `forespoergsler`, altså på Forespørgsler-fanen,
hvor den kunne **tælles, få en reference, lægges i kalenderen og
ikke blive væk**. En mail lander i en indbakke.

Det blev sagt højt, før siden blev skrevet om, og kunden holdt fast.
Så det er hans beslutning — men den betyder, at cateringens sager
holder op med at stå i admin, og at ingen kan se, hvor mange der er
kommet ind.

### Knappen bærer et færdigskrevet brev

Formularens egne felter står som linjer i mailen:
...
Adresse (ved levering):
Tidspunkt:
Ønsker til maden:
Allergier og hensyn:
```

Uden brevet ville personalet få *"hej, hvad koster catering?"* og
skulle ringe for at spørge om alt det, en formular spurgte om på ét
skærmbillede. Samme greb som forsidens selskabsknap fik samme dag:
`data-post` + `data-emne` + `data-brev`, hvor `js/skal/kontakt.js`
bytter adressen, hvis ejeren skriver en anden i admin → Kontakt.
`href` er reserven, så knappen virker uden JavaScript.

**⚠️ `data-post="booking"` er kundens eget valg.** `VEJLEDNING.md`
deler adresserne efter ÆRINDE — `booking@` er spørgsmål om en
booking, gæsten allerede HAR, og `selskab1@` er tilbud. Han bad
udtrykkeligt om booking her.

**⚠️ Og der er ÉN mailvej, ikke to.** `.anden-vej` har kun
telefonen. To mailadresser for det samme ærinde ville være to
postkasser at vælge imellem.

### Det sælgende har en kilde under hver linje

Forretningens egne ord om sig selv må siden gerne sige. Det, der
IKKE står der, er tal og faciliteter, ingen har bekræftet: antal år,
antal selskaber, stjerner, en pris pr. kuvert. Huset har betalt for
den lektion én gang — designbundtet fra 21/8 leverede *"4,8 · 312
anmeldelser"* og *"Bedste fiskefilet på hele Sydkysten"*.

Maden er den ene ting, der ER dokumenteret: hele sortimentet står i
menukortet med ejerens egne priser. Derfor peger afsnittet på kortet
i stedet for at love noget selv.

### Billederne er ikke kommet endnu

Der er ingen `data-fil` på pladserne — kun pladserne. Uden et foto
tegner `js/skal/billedplads.js` en flade i havnens farver med
pladsens eget tegn. En stiplet grå kasse ligner en side, der er gået
i stykker.

**⚠️ Pladserne har fået felter i admin** (`foto_catering_1-3` i
`FOTO_PLADSER`). Uden dem kunne fotoerne kun lægges ind ved at rette
i koden, og husets regel er, at ejeren selv skifter sine billeder.

### `js/skal/catering.js` er sidens ene script

Motoren `js/skal/forespoergsel.js` gjorde tre ting på siden:
formularen, billedpladserne og leveringsteksten. De to sidste er
flyttet til en lille fil, der hverken skriver eller henter noget
nyt.

**Leveringslinjen lovede opstilling, ingen har bekræftet**, og ikke
ét ord om hvor eller hvad det koster. Den skrives af
`Butik.leveringsTekst` nu — den SAMME funktion, forsiden og
smørrebrødssiden spørger. **Er levering slået fra i admin, ryger
hele rækken:** en faktalinje med *"Vi leverer"* er et løfte på
ejerens vegne.

### `SIDER.cdato` er slettet, ikke efterladt

Otte JavaScript-filer i repoet indlæses ikke af én eneste side
(30/8), og de er en fælde for den, der læser koden om et halvt år og
tror, de kører. En død `SIDER`-nøgle er den samme fælde i lille.

**⚠️ Men typen `catering` lever videre** i databasen
(`forespoergsel_type_ok`), i `FORESPOERGSEL_TYPER` i `store.js` og på
Forespørgsler-fanen. Gamle cateringforespørgsler skal stadig kunne
åbnes, aftales og afvises.

### Dækningen flyttede med — den forsvandt ikke

Hver regel, cateringens prøver bar, har fået et hjem på en side, der
stadig kører:

| Regel | Hvor den måles nu |
|---|---|
...
| nettet er datovælger | alle tre |
| ud af huset optager ingenting | `h-selskaber`, `h-frokost` |

De to fjernede prøver har hver sin note om, hvor reglen gik hen.
**Dækning forsvinder ikke ved, at en prøve fejler — den forsvinder
ved, at filen holder op med at blive kørt** (30/8).

**⚠️ Og mappeprøven er delt i to lister:** `forespoergselsSider()`
(tre, med formular) og `afleveringsSider()` (fire, cateringen med).
Kortet *"Sådan går det videre"* gælder dem alle; den ene røde knap
og `.anden-vej`s to hvide gælder kun dem med en formular. Lagt i den
samme løkke ville cateringen falde på en formular, den med vilje
ikke har — og så ville nogen lempe prøven for alle fire.

## Det sælgende kom på alle faner (4/9)

Kundens ord: *"det sælgende må godt komme på alle faner og gøre dem
flotte og pæne med animationer og frokostordninger og det hele."*
**Ingen SQL.**

Selskaber, smørrebrød ud af huset og frokostordningen har fået det
samme afsnit som cateringen, hver med sine egne ord.

### Og det er en prøve nu, ikke en god hensigt

`tests/gennemgang.spec.js` læser hvert `.saelg`-afsnit og fælder:

- en anmeldelsesscore (`\d[,.]\d` foran *på Google* eller *stjerner*)
- et antal anmeldelser
- et antal år, ingen har bekræftet
- en pris pr. kuvert, person eller medarbejder
- ordet *bedste*
- et antal afholdte selskaber, fester eller arrangementer

**⚠️ Mønstrene er brede med vilje.** Ikke *"4,8"*, men ETHVERT tal
foran *stjerner*: et fast tal holder op med at måle, første gang
nogen skriver et andet.
...
**⚠️ Prøven måler KUN `.saelg`.** Designets EGNE pladsholdere (4,8 på
Google, 40 pers., 199 kr. pr. person) står live på Mikkels
udtrykkelige beslutning fra 23/8, og designbundt-vagten er parkeret
imens. Den her vogter det, VI skriver — og afsnittene læses af
MAPPEN, så en femte side ikke kan udgives uden vagten.

### Animationen er designets egen, med ét trin mere

Punkterne i "Det får I" toner ind forskudt. **⚠️ Forsinkelsen sidder
på PUNKTET, ikke på en ny iagttager** — to iagttagere på det samme
element har allerede kostet en runde (den flydende pille, 31/8).
Reglen hænger på den `.in`, `.rev` allerede sætter.

**⚠️ Og den anden halvdel er den vigtige:** punkterne begynder på
`opacity: 0`. Virker `prefers-reduced-motion`-blokken ikke, står
listen som en TOM flade for netop den, der har bedt om mindre
bevægelse. Prøven måler det UDEN at rulle — ruller man først, redder
`.in` den, og prøven måler ingenting.

**⚠️ Et inline-link i brødtekst er 17 px højt**, og gennemgangen
fælder alt under 30. Rettelsen er husets egen fra 31/8: **lodret
padding med negativ margin**, så trykfladen vokser, mens linjens
højde står stille. IKKE `display:inline-block` — dét limede
footerens links sammen til *"Bestil madMenukort"* på syv sider.

## Den mørke sektion er historiens alene (4/9)

Kundens ord: *"efter skal vi have taget den der brune section vi har
med ting — det skal kun være til deres historie man kan læse om."*
**Ingen SQL.**

Sektionen var tre ting på én gang: *"Hvem er vi?"* med manchet og
underskrift, historien om ankeret, og tre værdikort (udsigten, 15 år
på havnen, grillmad og is). Husets egen regel siden 23/8 er ÉN ting
pr. afsnit — og et afsnit, der siger tre, læses som en
indholdsfortegnelse.

**⚠️ Og de to tal, der røg med, var designbundtets egne.** *"I 15 år
har vi lavet mad til havnens gæster"* og kortet *"15 år på havnen"*
kom fra bundtet 21/8 sammen med de 312 anmeldelser. Ingen har
bekræftet dem, og de står IKKE på historiesiden — netop fordi vi
ikke ved det.

**⚠️ Manchetten er historiesidens egen, ord for ord.** Skrev vi en ny
her, ville de to sider langsomt sige hver sit om den samme havn — og
gæsten går imellem dem i ét klik.

De tre kort er **kapitlerne** (1710 · Søslaget, Dengang · Ishuset,
Nu · Havnen) og siger kun det, der står på historiesiden. **De er
ikke links:** sektionens ene handling er knappen, og tre kort, der
førte samme sted hen, ville være fire veje til den samme side.
...
**⚠️ Og mønstret i prøven rammer påstanden, ikke ethvert årstal.**
Historien SKAL kunne sige *"1710"* og *"i næsten 270 år, før det
blev fundet"* — det er kilderne. Det forbudte er en påstand om, hvor
længe FORRETNINGEN har ligget her.
...
| `supabase/levering-og-mindsteantal.sql` | **Fragten som tal** (`leverings_gebyr` 79), postnumrene, sætningen uden 200-kroners-reglen og **mindst fire smørrebrød**. Fire rækker data — ingen nye kolonner, og intet tjek i `er-vi-klar.sql` (et tjek på ejerens eget tal ville sige ❌ den dag, han retter det) |
| `supabase/dato-vaern-resten.sql` | ⚠️ **KØR DEN FØR DEN NÆSTE.** Samme rettelse som `bestilling-dato-vaern.sql`, nu for `bordbestillinger`, `forespoergsler` og `udlejninger`. Uden den kan en booking fra i forgårs ikke sættes til Ankommet, en forespørgsel med en passeret dato ikke lukkes, og en overstået udlejning ikke bekræftes — alle tre er opdateringer, og hvert CHECK efterprøves på hele rækken |
...
## Når det går galt: fire tilstande, der skal måles

Stresstesten (4/9) målte meget data og en TOM database.
`vaerktoej/naar-det-gaar-galt.js` måler den tredje og fjerde: at
databasen svarer med en **fejl**, eller slet ikke svarer. Det er
ikke teoretisk — Supabase har nedetid, telefonen mister dækning
på molen, og en tabel kan mangle en kolonne, fordi en SQL-fil
ikke er kørt. Det er sket fire gange i det her projekt.

Værktøjet åbner hver udgivet gæsteside i fire tilstande:

| Tilstand | Hvad den efterligner |
|---|---|
| alt virker | målestokken |
| databasen svarer 500 | Supabase er nede |
| databasen svarer ikke | ingen dækning |
| en kolonne mangler (42703) | en SQL-fil er ikke kørt |

og leder efter blanke sider, rå fejlbeskeder, engelske ord og
JS-fejl. **Resultatet 5/9: de tretten sider, der overhovedet
spørger databasen, holder i alle fire tilstande** — ingen blank
side, ingen rå fejl, ingen JS-fejl.

**⚠️ MEN DEN MÅLTE INGENTING I FØRSTE KØRSEL.** Globalen hedder
`MOSEDE_CLOUD`, ikke `MOSEDE_CONFIG`, så `SKY` stod på false,
siderne kørte i øvetilstand, og alle tre fejltilstande meldte
*"0 af 14 sider har noget"*. Værktøjet tæller nu kaldene til den
falske adresse og **råber op, hvis en runde aldrig ramte
databasen**. Det er husets ældste ar: *en måling, der ikke rammer
det, den måler, siger "bestået".*

### Det, den fandt

**En nede database sagde, at gæstens bestilling ikke fandtes.**
`min-bestilling/` svarede *"Vi kan ikke finde en bestilling med
den reference"* — usandt: bestillingen findes, vi kan bare ikke
spørge. Og værre: siden **stoppede takten**, så den aldrig kom
sig igen, heller ikke da forbindelsen kom tilbage.

Roden var, at `Butik.bestillingStatus` gjorde
`r.ok ? r.json() : null` og havde et `.catch`, der også svarede
null. *"Svarede nej"* og *"kunne ikke spørge"* var altså det
samme svar. Nu kaster den en fejl med `ikkeSvar`, og siden
skelner.

**⚠️ Det lækker ingenting.** De TRE udfald, der med vilje skal
ligne hinanden (ukendt, slettet, for gammel), gør det stadig — de
kommer alle tre som en tom liste fra security definer-funktionen.
En netfejl siger intet om, OM referencen findes.

**⚠️ Og beskeden lover ikke, at bestillingen findes.** Siden kan
åbnes med en hvilken som helst reference, så *"din bestilling er
sendt"* ville være en påstand om noget, vi ikke ved.

**Statuspillen lovede en åbningstid, ingen havde sat.**
`startdata()` i `js/store.js` har 10-20 hver dag, så siden ikke
står tom. Med databasen nede sagde pillen *"Åbent nu til 20.00"*
— og en gæst, der kører til havnen kl. 19.45 på det løfte, har
spildt turen. Flaget fandtes i forvejen: `hent()` sætter
`_offline` på det, den falder tilbage på. Det havde bare ingen
læsere.

**⚠️ Resten af siden bliver stående.** Et menukort, der er en dag
gammelt, er bedre end en tom side. Det er kun PÅSTANDEN OM NUET,
der ikke må komme fra et tal i koden.

**Admin kunne skrive reservedata ind over ejerens egne.** De syv
lister råber hver især "kunne ikke hentes" — men
indstillingsfelterne stod pænt udfyldt med navn, adresse, telefon
og åbningstider fra `startdata()`. Et tryk på Gem ville skrive
dem ind over hans rigtige, og **autogem ville gøre det uden et
tryk overhovedet**, 1,2 sekund efter han rørte et felt.

**⚠️ Spærren hører i `skriv()`, ikke i `Admin.gem`.** Første
udgave sad dér — men fanerne bygger deres løfte FØR de kalder
`gem()`, så PATCH'en var allerede sendt, når spærren sagde nej.
Målt: en PATCH på `lokationer` slap ud, mens skærmen sagde "der
kan ikke gemmes". `skriv()` er det ENE sted, hver eneste rettelse
fra personalesiden går igennem. **Gæstens bestilling er ikke
...
`vaerktoej/tilgaengelighed.js` åbner hver gæsteside i to profiler
**og alle sytten admin-faner** og leder efter dobbelte id'er,
felter uden etiket, knapper uden navn, billeder uden alt,
overskrifter der springer et niveau over, kontrast og sidens eget
hoved.

**⚠️ FØRSTE KØRSEL GAV 190 FUND, OG DE FLESTE VAR MÅLINGENS EGNE
FEJL.** Fire ting kan ikke måles naivt, og en måling, der lader
som om, er værre end ingen:
...
- **en gradient eller et billede kan ikke måles som baggrund.**
  Designets røde knapper er en `linear-gradient`, så
  `backgroundColor` er gennemsigtig, og målingen gik OP til
  sidens creme: 1,00:1 på hver eneste knap
- **en baggrund kan ligge i et `::before`.** Heroen tegner sit
  tern og sin mørke tone dér, og `getComputedStyle` på elementet
  selv ser dem ikke
- **`innerText` er tom for et element, der ikke tegnes.** Otte
  knapper i en lukket fold blev meldt "uden navn"

Med de fire rettelser står den på **20 fund**, og de tyve er
overskriftsniveauer på designsiderne (se nedenfor).

### Den dæmpede tekst var ulæselig på ti sider

Designets `--muted #8b7871` giver **3,93:1 på creme, 3,61:1 på
cream2 og 4,18:1 på hvid**. Alle tre er under kravet på 4,5:1, og
det rammer `.fine`, `.hint`, `.tcap`, menukortets datolinje og
kalenderens manchet — netop den lille skrift, der er sværest at
læse i forvejen.

`css/style.css` fik den runde 22/8 og igen 29/8;
`havnegrillen.css` fik den aldrig, så fejlen har ligget der,
siden designet kom 23/8. Farven er nu **#6f5b55**, den SAMME som
det andet ark bruger (5,97:1). De to ark bærer hver sin halvdel
af gæstesiderne, og gæsten går imellem dem i ét klik — to
forskellige dæmpede farver ville være to udgaver af den samme
regel. **Reglen er en prøve nu**, målt på hver udgivet side.

### 47 felter i admin hed ingenting

Hverken etiket, aria-label eller en `<label>` om sig. For en
skærmlæser hedder sådan et felt *"redigeringsfelt"* — og det er
ikke kun for den, der ikke ser: en stemmestyring kan heller ikke
ramme "prisfeltet", når feltet ikke hedder noget. Personalet
sidder ved skærmen hele dagen.

**⚠️ Navnet siger RÆKKEN med.** "Pris" alene er tvetydigt på et
kort med 262 varer og syv gange under hinanden i åbningstiderne.
Derfor *"Pris på Flæskestegssandwich"*, *"Åbner mandag"*,
*"Intern note på forespørgslen fra Anna Vind"*.
...
Fundet af PRØVEN, ikke af værktøjet: prøven har data på fanerne.

### Overskriftsniveauerne røres ikke — og hvorfor

Tyve steder springer designsiderne fra `h1` til `h3`. Det er en
rigtig (om end lille) gene for den, der navigerer efter
overskrifter. Det rettes alligevel ikke: designet er et
1:1-handoff, kunden har sagt god for det, og at skifte tags på ti
sider er en DOM-ændring med visuel risiko mod en lille gevinst.
**Admins egen** (`h2 → h4` på Borde) ER rettet — den er vores og
ikke designets.

## Pengesporet: skærmen og databasen skal sige det samme

Prøverne dækker hver regel for sig — emballagen har sin, fragten
har sin, tillægget har sin. Men **summen kan være forkert, selv
om hver regel er rigtig**; det er husets lære fra pillen oven i
heroens manchet og fra galleriet, der kun gik op på en telefon.

`tests/pengesporet.spec.js` måler kæden hele vejen: læg noget i
kurven, læs **beløbet på skærmen**, send — og læg den **gemte
rækkes** linjer sammen. To tal, to steder.

**⚠️ Ét af tallene kommer udefra.** Prøven regner ikke summen med
de samme funktioner som siden, og reglen om hvad der er en RET
(emballagen er penge, ikke arbejde) står skrevet i prøven selv:
den må ikke spørge den funktion, den skal kontrollere.

Målingen fandt noget, der ikke blev ledt efter: sumlinjen skrev
*"kl. 15:00"* med kolon, mens tidsvælgeren lige over — og
kvitteringen, og `bestil/`, og `bord/` — skriver punktum.
