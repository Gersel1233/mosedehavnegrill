/* Fanen Dagens ret: ugeplanen og hurtigfeltet. Se js/admin/kerne.js
   for de to principper der gælder i alle admin-filerne.

   EGEN FANE 29/8 — kundens ord med spiis' admin som forlæg: dagens
   ret skal være "en sektion helt for sig selv, som hænger sammen
   med retterne". Ugeplanen og forgænger-feltet boede nederst på
   Forside-fanen, hvor ingen ledte efter dem: retten skrives HVER
   morgen, og ugen planlægges om mandagen — det er dagens arbejde,
   ikke hjemmesidens pynt. Koden her er flyttet 1:1 fra
   js/admin/forside.js; kun "i dag"-mærket på ugens første dag er
   nyt. Forside-fanen beholder kuglerne og billederne. */
(function () {
  'use strict';

  var $ = Admin.$;

  function tegnDagensRet() {
  ...
  function dagKort(iso, erIDag) {
    ...
    /* "I DAG ·" er pænDatos eget mærke — den sætter det selv på
       dagens dato, for hele admin. Et mærke mere her stod som
       "I DAG · Lørdag … [I dag]" og sagde det samme to gange;
       MÅLT på et skud, ikke læst. erIDag-flaget beholdes til den
       dag, kortet skal noget mere end teksten. */
    hoved.appendChild(Admin.lav('strong', null, Admin.pænDato(iso)));
